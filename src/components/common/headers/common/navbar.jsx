...
class NavBar extends Component {
	...
	handleMenuCategoryClick = (e) => {
		document.querySelectorAll(".subCategory a").forEach(function(item) {
			// item.classList.remove("text-color");
			item.style.color = "black";
		});
		if (e.target.style == "color:#ff4c3b") {
			return;
		} else {
			// e.target.classList.add("text-color");
			e.target.style.color = "#ff4c3b";
		}
	};
								...
								<ul className="nav-submenu">
									{this.state.categories.map((item) => (
										<li key={item.category_id} className="subCategory">
											<a onClick={(e) => this.handleMenuCategoryClick(e)}>
												{item.name}
