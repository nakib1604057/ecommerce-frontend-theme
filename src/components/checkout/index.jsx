...
import { removeFromWishlist, clearCart } from "../../actions";
  ...
  componentDidMount() {
    ...
    getShippingCost(strProductIds).then(data => {
      ...
      this.setState({
        ...
        shippingCost: this.getShippingPrice(data.data.data, this.state.city),
        totalCost:
          this.props.total +
          this.getShippingPrice(data.data.data, this.state.city),
      });
    });
  }

  getShippingPrice = (shipping, city) => {
  ...
  render() {
    ...
    return (
      <div>
        {/*SEO Support*/}
        <Helmet>
          <title>Notlens | CheckOut Page</title>
          <meta
            name="description"
...
export default connect(mapStateToProps, { removeFromWishlist, clearCart })(
  checkOut
);
